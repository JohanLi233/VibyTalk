...
class MobileOneBlock(nn.Module):
    ...
    def reparameterize(self):
        ...
        self.reparam_conv = nn.Conv2d(in_channels=self.in_channels,
                                      ...
                                      stride=self.stride,
                                      padding=self.kernel_size // 2 if hasattr(self, 'rbr_conv') else 0,
                                      dilation=1,
                                      groups=self.groups,
                                      bias=True)
        self.reparam_conv.weight.data = kernel
        if self.reparam_conv.bias is not None and bias is not None:
    ...
    def _get_kernel_bias(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        kernel_scale = torch.zeros((self.out_channels, self.in_channels // self.groups, self.kernel_size, self.kernel_size))
        bias_scale = torch.zeros(self.out_channels)
        if self.rbr_scale is not None:
        ...
        kernel_identity = torch.zeros((self.out_channels, self.in_channels // self.groups, self.kernel_size, self.kernel_size))
        bias_identity = torch.zeros(self.out_channels)
        if self.rbr_skip is not None:
            kernel_identity, bias_identity = self._fuse_bn_tensor(self.rbr_skip)

        # get weights and bias of conv branches
        kernel_conv = torch.zeros((self.out_channels, self.in_channels // self.groups, self.kernel_size, self.kernel_size))
        bias_conv = torch.zeros(self.out_channels)
        for ix in range(self.num_conv_branches):
    ...
    def _fuse_bn_tensor(self, branch) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if isinstance(branch, nn.Sequential):
            conv_layer = branch[0]  # conv layer
            bn_layer = branch[1]    # bn layer
