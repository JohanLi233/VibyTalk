...
class PFLD_GhostOne(Module):
    def __init__(
        ...
        self.conv5_1 = GhostOneBottleneck(
            int(96 * width_factor),
            int(336 * width_factor),
            int(144 * width_factor),
            stride=2,
            inference_mode=self.inference_mode,
            num_conv_branches=self.num_conv_branches,
        )
        ...
        self.conv_out = nn.Conv2d(
            int(512 * width_factor), landmark_number * 2, 1, 1, 0
        )  # 这个大小需要改
...
class PFLD_GhostOne_WithSTN(Module):
    def __init__(
        ...
        super(PFLD_GhostOne_WithSTN, self).__init__()
...
class AuxiliaryNet(Module):
    def __init__(self, width_factor=1):
        ...
        self.conv4 = Conv_Block(
            int(144 * width_factor), int(64 * width_factor), 1, 1, 0
        )

        self.merge1 = Conv_Block(
            int(64 * width_factor), int(64 * width_factor), 3, 1, 1
        )
        self.merge2 = Conv_Block(
            int(64 * width_factor), int(64 * width_factor), 3, 1, 1
        )
        self.merge3 = Conv_Block(
            int(64 * width_factor), int(64 * width_factor), 3, 1, 1
        )
    ...
    def forward(self, out1, out2, out3, out4):
        ...
        up4 = F.interpolate(
            output4, size=[output3.size(2), output3.size(3)], mode="nearest"
        )
        output3 = output3 + up4
